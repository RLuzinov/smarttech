/******/ (() => { // webpackBootstrap
/*!*********************************************!*\
  !*** ./src/block-our-partners-main/view.js ***!
  \*********************************************/
// view.js
(function () {
  ...
  function initTabs() {
    document.querySelectorAll(".partner__tab-btn").forEach(btn => {
      btn.addEventListener("click", function () {
        ...
        document.querySelectorAll(".partner__tab-btn").forEach(b => b.classList.remove("partner__tab-btn--active"));
  ...
  function drawLines() {
    ...
    grid.querySelectorAll(".partner__v-line--top-between, .partner__v-line--bottom-between, .partner__h-line--top-of-row, .partner__h-line--bottom-of-row, .partner__v-line--dynamic").forEach(el => el.remove());
    ...
    for (let r = 1; r < rows.length; r++) {
      ...
      if (lineY >= 0) {
        ...
        addDynamicVerticalLine(linesContainer, "partner__v-line--top-left", lineY - 19, minLeft);
        addDynamicVerticalLine(linesContainer, "partner__v-line--top-right", lineY - 19, maxRight);
      }
    }
    for (let r = 0; r < rows.length - 1; r++) {
      const currentIndices = rows[r].items;
      const rowBottom = Math.max(...currentIndices.map(i => itemRects[i].bottom));
      const lineY = rowBottom + BOTTOM_OFFSET;
      const minLeft = Math.min(...currentIndices.map(i => itemRects[i].left));
      const maxRight = Math.max(...currentIndices.map(i => itemRects[i].right));
      if (lineY <= gridRect.height / zoom) {
        ...
        addDynamicVerticalLine(linesContainer, "partner__v-line--bottom-left",
        // lineY + bottomOffset,
        lineY, minLeft);
        addDynamicVerticalLine(linesContainer, "partner__v-line--bottom-right",
        // lineY + bottomOffset,
        lineY, maxRight);
      }
    }
